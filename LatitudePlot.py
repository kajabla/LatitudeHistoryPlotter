...
def GetKmlFiles():
    """Locates and reads local .kml files, returns a list of kml dictionary
        data"""
    KmlData = []
...
def ReadKmlFile(dirname, filename):
    """Parses a single kml file, returns a dict of format {time: [lat,
                                                                  long]}"""
    KmlData = {}
    ...
    while line:
        if 'when' in line:
            timelist.append(time.strptime(ET.fromstring(line)[0].text,
                                          "%Y-%m-%dT%H:%M:%SZ"))
...
def DrawMapData(KmlData, InputImage, OutputImage, itop, ibottom, ileft, iright,
                xnudge, ynudge):
    ...
    for KmlD in KmlData:
        for d in range(len(KmlD[0])-1):
            ...
            if(EuclidDistance(x1, y1, x2, y2) < 10000):
                # setting this around 80 works okay. Attempts to remove some
                # noise
                draw.line((x1, y1, x2, y2), fill=(214, 13, 13))
            cnt += 1
            if cnt % 10000 == 0:
...
def ScalingFunc(inputv, minv, maxv, size):
    ...
    if((float(maxv) - float(minv)) == 0):
        return 0
    return ((((float(inputv) - float(minv)) / (float(maxv) - float(minv)))
            * float(size)))
...
if __name__ == "__main__":
    ...
    DrawMapData(GetKmlFiles(), ImageData[0], "LatitudeData.png", ImageData[1],
                ImageData[2], ImageData[3], ImageData[4], ImageData[5],
                ImageData[6])
